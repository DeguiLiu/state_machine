...
const SM_Transition T_Off[] = {
    {EV_POWER_ON, &STATE_Idle, NULL, NULL,
     SM_TRANSITION_EXTERNAL}  // Power On -> Go directly to Idle
};
...
// --- State Definitions ---
static const SM_State STATE_Off = {.parent = NULL,  // Top-level state
                                   .entryAction = NULL,
                                   .exitAction = NULL,
                                   .transitions = T_Off,
                                   .numTransitions = sizeof(T_Off) / sizeof(T_Off[0]),
                                   .name = "Off"};
static const SM_State STATE_On = {.parent = NULL,  // Top-level state
                                  .entryAction = entry_On,
                                  .exitAction = exit_On,
                                  .transitions = T_On,
                                  .numTransitions = sizeof(T_On) / sizeof(T_On[0]),
                                  .name = "On"};
static const SM_State STATE_Idle = {.parent = &STATE_On,  // Child of 'On'
                                    .entryAction = entry_Idle,
                                    .exitAction = NULL,
                                    .transitions = T_Idle,
                                    .numTransitions = sizeof(T_Idle) / sizeof(T_Idle[0]),
                                    .name = "Idle"};
static const SM_State STATE_Running = {.parent = &STATE_On,  // Child of 'On'
...
/*
$ ./test_state_machine ✭master Initial state: Off Is in state 'On'? No (Entry)-> On (Entry)-> Idle
Current state: Idle
Is in state 'On'? Yes
