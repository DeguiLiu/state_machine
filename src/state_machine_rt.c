...
static void perform_transition_internal(SM_StateMachine* sm, const SM_State* target_state,
...
static const SM_Transition* find_matching_transition(const SM_State* state, const SM_Event* event,
                                                     bool* guard_passed, SM_StateMachine* sm);
static bool execute_transition(SM_StateMachine* sm, const SM_Transition* transition,
                               const SM_Event* event);
static bool process_state_transitions(SM_StateMachine* sm, const SM_State* state,
                                      const SM_Event* event);
static void execute_exit_actions(SM_StateMachine* sm, const SM_State* source_state,
                                 const SM_State* lca, const SM_Event* event);
static bool build_entry_path(SM_StateMachine* sm, const SM_State* target_state,
                             const SM_State* lca);
...
SM_RT_Result SM_RT_Init(SM_RT_Instance* rt_sm, const SM_State* initial_state,
    ...
    if (validation_passed) {
        ...
        sm_rt_init_base(&rt_sm->base_sm, initial_state, entry_path_buffer, buffer_size, user_data,
                        unhandled_hook);
...
SM_RT_Result SM_RT_IsInState(const SM_RT_Instance* rt_sm, const SM_State* state,
                             bool* is_in_state) {
    SM_RT_Result result = SM_RT_RESULT_ERROR_UNKNOWN;
    bool validation_passed = false;
...
static void sm_rt_init_base(SM_StateMachine* sm, const SM_State* initial_state,
    ...
    /* Parameter validation */
    if ((sm != NULL) && (initial_state != NULL) && (entry_path_buffer != NULL) &&
        (buffer_size > 0U)) {
        valid_parameters = true;
    }
    ...
    if (valid_parameters) {
        ...
        sm->currentState =
            NULL; /* Setting to NULL ensures the full entry path is executed on first transition */
...
static const SM_Transition* find_matching_transition(const SM_State* state, const SM_Event* event,
    ...
    *guard_passed = false;
    ...
    if ((state != NULL) && (state->transitions != NULL) && (state->numTransitions > 0U) &&
        (event != NULL)) {
        ...
        while ((transition_idx < state->numTransitions) && (!found)) {
            ...
            if (current_transition->eventId == event->id) {
                ...
                *guard_passed =
                    (current_transition->guard == NULL) || current_transition->guard(sm, event);
            }
...
static bool process_state_transitions(SM_StateMachine* sm, const SM_State* state,
    ...
    if ((sm != NULL) && (state != NULL) && (event != NULL)) {
        ...
        if ((matching_transition != NULL) && guard_passed) {
            ...
            SM_LOG_DEBUG("Guard for event %u in state '%s' failed.", (unsigned)event->id,
                         state->name);
        }
    }
